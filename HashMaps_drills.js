...
//Simplified solution to number 6.
// function anagrams(words) {
// 	const ret = [], map = {};
// 	for (let word of words) {
// 		const key = word.split('').sort().join('');
// 		if (!map[key]) ret.push(map[key] = []);
// 		map[key].push(word);
// 	}
// 	return ret;
// }

//o(n) in the number of words in the dictionary
function anagram(arr) {
	...
	for (let i = 0; i < arr.length; i++) {
		let sortedWord = arr[i].split('').sort((a, b) => a > b).join('');

		try {
			hashMapKey = hashMap.get(sortedWord);
		}
		catch (err) {
			hashMapKey = null;
		}
		...
		if (hashMapKey === null) {
			hashMap.set(sortedWord, counter);
			counter++;
			result.push([arr[i]]);
		}
		else {
			let index = hashMapKey;
